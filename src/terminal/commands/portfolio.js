...
const contact = {
    ...
    run() {
        return [
            ...
            out.blank(),
            profile.available
                ? out.success("Open to network-focused opportunities.")
...
const resume = {
    ...
    run(ctx, { flags }) {
        if (flags.download) {
            ...
            return [
                out.success("downloading Maruf_Ahmed_CV.pdf…"),
        ...
        return [
            ...
            out.ascii(
                keyValue([
                    ...
                    ["live", p.liveLink],
                    ["source", p.githubLink],
                    ...(p.apiLink ? [["api", p.apiLink]] : []),
                    ...(p.serverGithubLink ? [["server source", p.serverGithubLink]] : []),
                ])
            ),
            out.blank(),
            ...
            ...section("Engineering focus", p.caseStudy.challenges),
            ...section("Result", p.caseStudy.result),
            ...(p.facts?.length
                ? [out.heading("Project scope"), ...p.facts.map((fact) => out.success(`  ${fact}`)), out.blank()]
                : []),
